...
The Liaron Core daemon can be configured to act as a trusted "border
router", implementing the liaron wire protocol and relay, making
...
The ZeroMQ feature in Liaron Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building liarond:
...
For instance:
    ...
    $ liarond -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/liarond.tx.raw \
...
These options can also be provided in liaron.conf.
...
From the perspective of liarond, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into liarond directly. Furthermore, no information is
...
using. Liarond appends an up-counting sequence number to each
