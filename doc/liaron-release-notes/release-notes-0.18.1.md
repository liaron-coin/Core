Liaron Core version 0.18.1 is now available from:

  <https://download.liaron.org/liaron-0.18.1/>
...
`/Applications/Liaron-Qt` (on Mac) or `liarond`/`liaron-qt` (on
Linux).
...
Liaron Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Liaron Core on unsupported systems.
...
10.10. Additionally, Liaron Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:18809:18809` (this is an extra `:18809` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@liaron.org) the Liaron Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 LRN),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in LRN per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/liaron-conf.md)
  about the `liaron.conf` file describes how to use it to configure
  Liaron Core.

- A new document introduces Liaron Core's BIP174 [Partially-Signed
  Liaron Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Liaron-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Liaron Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Liaron Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Liaron Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart liarond with the `-deprecatedrpc=generate`
...
- A new `liaron-wallet` tool is now distributed alongside Liaron
...
This section describes planned changes to Liaron Core that may affect
other Liaron software and services.

- Since version 0.16.0, Liaron Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Liaron Core 0.20 (expected about
  a year after 0.18), Liaron Core will default to native segwit
  ...
  to bech32 addresses, and if the Liaron Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Liaron Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Liaron Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Liaron Core
...
- A sub-project of Liaron Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Liaron Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Liaron Core's own implementation, although entropy
  gathered by Liaron Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Liaron Core a
...
- On macOS, Liaron Core now opts out of application CPU throttling
...
- #18809 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
